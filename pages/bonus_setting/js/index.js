...
async function handleDataTable() {
  ...
  let data = [];
  let tableData = [];
  let resDeopsit = await callXMLHttpRequest(`${apiPort.apiDataMemberPartner}`, {});
...
async function editMember(id, dataDetail) {
  ...
  let resData = await callXMLHttpRequest(`${apiPort.apiChildrenMemberPartner}`, data);
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) {
      let resDeleteMember = await callXMLHttpRequest(
        `${apiPort.apiDeleteMemberPartner}`,
        { id }
      );
...
async function handleSubmit() {
  ...
  const result = await callXMLHttpRequest(`${apiPort.apiMemberPartner}`, data);
...
async function blankStucture() {
  let resData = await callXMLHttpRequest(`${apiPort.apiGetMember}`, {});
  if (resData.statusCodeText == textRespone.ok.CodeText) {
    /** => add code  */
  } else {
    handleApiFaild(resData);
...
async function genObtionSelectMember(name = "member", val) {
  let resData = await callXMLHttpRequest(`${apiPort.apiGetMember}`, {});
  if (resData.statusCodeText == textRespone.ok.CodeText) {
...
async function genObtionSelectMemberPartner() {
  let resData = await callXMLHttpRequest(`${apiPort.apiGetMember}`, {
    partnerType: "PN",
  });
  if (resData.statusCodeText == textRespone.ok.CodeText) {

      ...
      tableData.push([
        ...
        data[i].total >= 0
          ? '<span style="color:green">' +
            formatMoneyNotDecimal(data[i].total) +
            "</span>"
        ...
        data[i].receive >= 0
          ? '<span style="color:green">' +
            formatMoneyNotDecimal(data[i].receive) +
            "</span>"
          : '<span style="color:red">' +
            formatMoneyNotDecimal(data[i].receive) +
            "</span>",
...
async function history(id, start_date, end_date) {
  ...
  let resDeopsit = await callXMLHttpRequest(
    `${apiPort.apiQueryDetailAlliance}`,
    ddd
  );
  if (resDeopsit.statusCodeText == textRespone.ok.CodeText) {
    data = resDeopsit.data;
...
async function clear_credit() {
  ...
  let resAlliance = await callXMLHttpRequest(
    `${apiPort.apiAddAllianceLog}`,
    data
  );
  if (resAlliance.statusCodeText == textRespone.ok.CodeText) {
    Swal.fire({
...
async function historyResult(username) {
  ...
  let tableData = [];
  let resDeopsit = await callXMLHttpRequest(`${apiPort.apiQueryAllianceLog}`, {
    username: username,
  });
  if (resDeopsit.statusCodeText == textRespone.ok.CodeText) {
    data = resDeopsit.data;
...
async function editMember(id) {
  $(".modal").modal("hide");
  let resMember = await callXMLHttpRequest(`${apiPort.apiQueryAllianceByID}`, {
    id: id,
  });
...
async function edit_alliance_modal() {
  ...
  let resEditMember = await callXMLHttpRequest(
    `${apiPort.apiUpdateMemberAlliance}`,
    data
  );
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) {
      let resDeleteMember = await callXMLHttpRequest(
        `${apiPort.apiDeleteMemberAlliance}`,
        { id: idDelete }
      );

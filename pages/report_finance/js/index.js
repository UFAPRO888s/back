...
async function setData() {
    ...
    let resFinance = await callXMLHttpRequest(
        `${apiPort.apiQueryFinanceReport}`,
        data
    );
    if (resFinance.statusCodeText == textRespone.ok.CodeText) {
        ...
        let deposit = 0;
        let withdraw = 0;
        let withdraw_cancle = 0;
        for (let i = 0; i < data.length; i++) {
            deposit = deposit + parseFloat(data[i].deposit);
            withdraw = withdraw + parseFloat(data[i].withdraw);
            withdraw_cancle = withdraw_cancle + parseFloat(data[i].withdrawcancle);
            ...
            htm += '<td class="text-center">' + formatMoney(data[i].withdrawcancle) + "</td>";
        ...
        htm += '<td class="text-center">' + formatMoney(withdraw_cancle) + "</td>";

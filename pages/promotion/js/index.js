    ...
    if (resPromotion.statusCodeText == textRespone.ok.CodeText) {
        ...
        for (let i = 0; i < data.length; i++) {
            ...
            tableData.push([
                ...
                data[i].name,
                data[i].percen + "%",
                data[i].x == 0 ? data[i].turn : data[i].x,
...
async function add_promotion() {
    if (canEdit) {
        ...
        var files = document.getElementById("url_img_1_add").files;
        if (files.length > 0) {
            var formData = new FormData();
            formData.append("dir", "webauto_promotion");
            ...
            if (res.imageUrl) {
                ...
                let resAlliance = callXMLHttpRequest(
                    `${apiPort.apiAddPromotion}`,
                    data
                );
...
function detail_promotion(id) {
    ...
    if (resPromotion.statusCodeText == textRespone.ok.CodeText) {
        ...
        // el('type_1').value = resPromotion.data[0].type;

        $("#type_1").val(resPromotion.data[0].type);
...
function edit_promotion_modal(id) {
    if (canEdit) {
        let resPromotion = callXMLHttpRequest(
            `${apiPort.apiQueryPromotionEditByID}`, { id: id }
        );
        if (resPromotion.statusCodeText == textRespone.ok.CodeText) {
            let htm = "";
            ...
            // console.log(resPromotion.data[0])

            el("pro_type_edit").innerHTML = htm;
            el("img_1_edit").src = resPromotion.data[0].img_name;
            el("url_file_1_edit").value = resPromotion.data[0].img_name;
            ...
            // el('type_1_edit').value = resPromotion.data[0].type;
            $("#type_1_edit").val(resPromotion.data[0].type);
...
async function edit_promotion() {
    if (canEdit) {
        let pro_type = el("pro_type_edit").value;
        let img_name = el("url_file_1_edit").value;
        ...
        var files = document.getElementById("url_img_1_edit").files;
        ...
        if (files.length > 0) {
            ...
            data.img_name = el("url_file_1_edit").value;
            let resPromotion = callXMLHttpRequest(
                `${apiPort.apiUpdatePromotionbyID}`,
                data
            );
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                let resDeleteMember = callXMLHttpRequest(
                    `${apiPort.apiDeletePromotion}`, { id: id }
                );

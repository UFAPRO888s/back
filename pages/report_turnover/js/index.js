...
function fetchDeposit(start_date, end_date, username) {
  let column = [
    {
      title: "ชื่อผู้ใช้งาน",
  ...
  let data = [];
  let tableData = [];
  let resTurnover = callXMLHttpRequest(`${apiPort.apiQueryTurnover}`, {
    start: start_date,
    end: end_date,
    username: username,
  });
  if (resTurnover.statusCodeText == textRespone.ok.CodeText) {
    ...
    for (let i = 0; i < data.length; i++) {
      tableData.push([data[i].username, data[i].turnover, data[i].date]);
    }
    el("res_start").innerHTML = start_date;
    el("res_end").innerHTML = end_date;

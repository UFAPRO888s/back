...
$(document).ready(async function() {
    await setDate("startDate", moment().startOf("isoWeek").format("YYYY-MM-DD"));
    await setDate("endDate", moment().endOf("isoWeek").format("YYYY-MM-DD"));
});
...
// async function tranferYod(username, amont, startDate, endDate) {
//     if (this.todayDate != 1 || (this.startLastWeek != this.startDate) || (this.endLastWeek != this.endDate)) {
//         return
//     }
//     await showLoader()
//     let data = {
...
//         date_new: moment().format('YYYY-MM-DD'),
//         amont: amont,
//         remark: CryptoJS.MD5(`${startDate}${endDate}${username}${amont}`)
//     };
//     let checkRemark = await callXMLHttpRequest(`${apiPort.apiCheckRemark}`, { remark: data.remark });
//     if (checkRemark.data.length > 0) {
//         Swal.fire({
//             title: 'แจ้งเตือน',
//             text: 'มีรายการโอนยอดไปแล้ว',
//             icon: 'error',
...
//         return
//     }
//     let result = await callXMLHttpRequest(`${apiPort.apiTranferYod}`, data);

//     if (result.statusCodeText == textRespone.ok.CodeText) {
//         Swal.fire({
//             title: 'สำเร็จ',
//             text: 'โอนยอดดรียบร้อย',
//             icon: 'success',
//             showCancelButton: false,
//             confirmButtonColor: '#3085d6',
//             cancelButtonColor: '#d33',
    ...
    methods: {
        async callAll() {
            ...
            let result = await callXMLHttpRequest(
                `${apiPort.apiMemberPartnerList}`,
                data
            );

            if (result.statusCodeText == textRespone.ok.CodeText && result.data) {
                // console.log(result.data)
                this.data_senior = result.data.data_senior;
                this.data_master = result.data.data_master;
                this.data_agent = result.data.data_agent;
                const chunkSize = 10;
                for (let i = 0; i < this.data_senior.length; i += chunkSize) {
                    this.chunk.push(this.data_senior.slice(i, i + chunkSize));
                }

                this.dataShow = this.chunk[this.page - 1];
                this.calPage(this.data_senior.length);
            } else {
                handleApiFaild(result);
            }
            this.hideLoader();
        ...
        async callMaster() {
            ...
            this.dataShow = [];
            const chunkSize = 10;
            for (let i = 0; i < this.data_master.length; i += chunkSize) {
                this.chunk.push(this.data_master.slice(i, i + chunkSize));
            }

            this.dataShow = this.chunk[this.page - 1];
            this.calPage(this.data_master.length);
            this.hideLoader();
        },
        async callAgent() {
            await this.showLoader();
            ...
            this.dataShow = [];
            const chunkSize = 10;
            for (let i = 0; i < this.data_agent.length; i += chunkSize) {
                this.chunk.push(this.data_agent.slice(i, i + chunkSize));
            }

            this.dataShow = this.chunk[this.page - 1];
            this.calPage(this.data_agent.length);
            this.hideLoader();
        },
        async callSubAgent() {
            await this.showLoader();
        ...
        checktranfer(val) {
            if (this.todayDate == 4 && ((this.startLastWeek == this.startDate && this.endLastWeek == this.endDate) || (this.startLastWeek2 == this.startDate && this.endLastWeek2 == this.endDate))) {
                return `btn btn-outline-primary text-right w-100`;
            } else {
                return `text-right table-warning`;
            }
        },
        async tranferYod(username, amont, startDate, endDate) {
            if (this.todayDate != 4 || (this.startLastWeek != this.startDate || this.endLastWeek != this.endDate) && (this.startLastWeek2 != this.startDate || this.endLastWeek2 != this.endDate)) {
                return;
            }
            if (amont == 0) {
            ...
            let data = {
                ...
                date_new: moment().format("YYYY-MM-DD"),
                amont: amont,
                remark: md5(`${startDate}${endDate}${username}${amont}`),
            };
            let checkRemark = await callXMLHttpRequest(`${apiPort.apiCheckRemark}`, {
                remark: data.remark,
            });
            console.log(checkRemark.data.length);
            if (checkRemark.data.length > 0) {
                Swal.fire({
                    title: "แจ้งเตือน",
                    text: "มีรายการโอนยอดไปแล้ว!!",

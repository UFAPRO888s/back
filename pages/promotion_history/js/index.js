...
function setDataPromotionHistory() {
  ...
  let resPromotionHistory = callXMLHttpRequest(
    `${apiPort.apiQueryePromotionHistory}`,
    {}
  );
  if (resPromotionHistory.statusCodeText == textRespone.ok.CodeText) {
    ...
    for (let i = 0; i < data.length; i++) {
      tableData.push([
        data[i].username,
        (data[i].wallet_balance * data[i].pro_percen) / 100,
        data[i].pro_trun_amount,
        formatDate(data[i].created_at) + " " + formatTime(data[i].created_at),
        data[i].name,
      ]);
    }

...
function setDataResult() {
    ...
    if (resResults.statusCodeText == textRespone.ok.CodeText) {
        ...
        if (resResults.total_today >= 0) {
            el('total_today').innerHTML = '<font color="white">' + formatMoneyNotDecimal(resResults.total_today) + '</font> บาท';
        } else {
        ...
        if (resResults.total_month >= 0) {
            el('total_month').innerHTML = '<font color="white">' + formatMoneyNotDecimal(resResults.total_month) + '</font> บาท';
        } else {
        ...
        if (resResults.total_year >= 0) {
            el('total_year').innerHTML = '<font color="white">' + formatMoneyNotDecimal(resResults.total_year) + '</font> บาท';
        } else {
        ...
        for (let i = 0; i < resResults.last_deposit.length; i++) {
            ...
            htm_last_deposit += '<td style="text-align:center;">' + formatDate(resResults.last_deposit[i].transaction_date) + ' ' + formatTime(resResults.last_deposit[i].transaction_date) + '</td>';
            htm_last_deposit += '<td style="text-align:center;">' + formatMoneyNotDecimal(resResults.last_deposit[i].amount) + '</td>';
            htm_last_deposit += '</tr>';
        }

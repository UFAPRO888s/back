...
function setWheel() {
  if (canEdit) {
    ...
    let resSettingWheel = callXMLHttpRequest(
      `${apiPort.apiQueryeSettingWheel}`,
      {}
    );
    if (resSettingWheel.statusCodeText == textRespone.ok.CodeText) {
      ...
      for (let i = 0; i < data.length - 1; i++) {
        ...
        htm += '<div class="row">';
        htm +=
          '<div class="col-xs-12 col-md-1" style="text-align: center;padding: 15px;">';
        htm +=
          '<label for="" style="text-align: center;">วงที่ ' +
          (i + 1) +
          "</label>";
        htm += " </div>";
        ...
        htm +=
          '<input type="text" class="form-control cc-name" value="' +
          data[i + 1].value +
          '" id="value_' +
          (i + 1) +
          '" required>';
        ...
        slices[i].text =
          data[i + 1].value != 0
            ? data[i + 1].value + " เครดิต"
            : "ไม่ได้เครดิต";
        slices[i].des = data[i + 1].des;
        slices[i].value = data[i + 1].value;
      }
      htm +=
        '<button onclick="setting_wheel()" class="btn btn-lg btn-info btn-block">บันทึก</button>';

...
const en = function (name) {
  return document.getElementsByName(name);
};

Delay = (milli) =>
  new Promise((resolve) => setTimeout((_) => resolve(), milli));
...
function onErrorImage(e) {
  e.src =
    "//s3auto.sgp1.vultrobjects.com/js_wauto/source/img/empty.jpg";
}
...
function generateOptionBank(selection, data, isPlaceholder) {
  ...
  for (let i = 0; i < data.length; i++) {
    strHTML +=
      '<option  value="' + data[i]._id + '">' + data[i].bank_name + "</option>";
  }
  document.getElementById(selection).innerHTML = strHTML;
  $("#" + selection).selectpicker("refresh");
}
...
const getParamInGetUrl = function () {
  const objectData = {};
  const param_array = window.location.href.split("?")[1]
...
const checkAndUpdateInGame = function () {
  ...
  if (getToken) {
    ...
    callXMLHttpRequest(`${apiPort.apiGateWay}/playGame/updateStatusInGame`, {
      uid: uid,
      ou_id: ou_id,
    });
  }
...
function getScript(nameScript, callback) {
  $.getScript(
    `${window.location.origin}/gpp/js/${nameScript}.js`,
  ...
  data.forEach(function (element) {
    if (element.param_default === "1") {
      ...
      option +=
        ...
        element.name +
        " || ราคา " +
        formatMoneyNotDecimal(element.price) +
        " เครดิต" +
        "</option>";
    }
  });
...
function convertTHDateNoTime(dateTh) {
  const dateThArr = dateTh.split("/");
  const dateEn = `${dateThArr[2]}-${dateThArr[1]}-${dateThArr[0]}`;
  return dateEn;
}
...
function convertMember(username) {
  let result = callXMLHttpRequest(`${apiPort.apiConvertMember}`, { username });
...
function convertMemberPartner(username) {
  let result = callXMLHttpRequest(`${apiPort.apiConvertMemberPartner}`, {
    username,
  });
  if (result.statusCodeText == textRespone.ok.CodeText) {
    return result.data;
